...
1. obtain 70GB tar db [https://home.gouin.io:5443/0L.167.tar.xz](https://home.gouin.io:5443/0L.167.tar.x)
2. untar it to your `db` dir set in your config file.
3. checkout network reboot release
   ```console
   $ git fetch && git checkout release-v5.1.0 -f
   ```
4. make sure everyone is on the same commit
   ```console
    $ git log
    commit 43717da26aa9f008ca31b1e5c0a7f26e81b7a39b
   ```

5. export Vals list for the intial network (TODO: What IS THIS???)
   ```console
   ...
   ```
7. build the necessary tools, `make tx`
   ```
...
## Start diem node with make start or  diem-node -f ~/.0L/validator.node.yaml >> ~/.0L/logs/validator.log 2>&1

```
## Validator.yaml seeds
...
```console
$ ol -a ACCOUNT query --epoch
EPOCH 169 - WAYPOINT: 37235489:b019393e57fdb375143d1d864843362c393ee55d4123bdd2cf0ec7a088a500f2

$ ol -a ACCOUNT query --balance
